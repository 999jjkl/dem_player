...
from PySide6.QtGui import (
    ...
    QPixmap, QImage, QImageReader, QIcon, QAction, QGuiApplication,
    QTextCursor, QTextCharFormat
)

APP_VERSION = "0.19.10"
# 0.19.10+: migrated from PyQt6 to PySide6

QWIDGETSIZE_MAX = 16777215

CUSTOM_BG_MAX_BYTES = 5 * 1024 * 1024
CUSTOM_BG_MAX_DIM = 3840
...
CUSTOM_BG_FILENAME = "custom_background.png"
CUSTOM_BG_PATH = os.path.join(BACKGROUND_DIR, CUSTOM_BG_FILENAME)
...
def vlc_list_audio_devices(vlc_instance_ptr, aout=b"mmdevice"):
    ...
    lib = _load_libvlc()
    if lib is None:
        log("[VLC] Cannot load libvlc.dll for device enumeration", 30)
        return results
    try:
        ...
        log(f"[VLC] Device enumeration failed: {e}", 30)
    return results
...
def vlc_set_audio_device(vlc_player, device_id):
    ...
    lib = _load_libvlc()
    if lib is None:
        log("[VLC] Cannot load libvlc.dll for device set", 30)
        return False
    try:
        ...
        log(f"[VLC] Set device failed: {e}", 30)
        return False
    ...
    try:
        ...
        for i in range(count):
            ...
            if bass.BASS_GetDeviceInfo(i, ctypes.pointer(info)):
                name = info.name.decode("utf-8", errors="replace") if info.name else f"Device {i}"
                results.append((i, name))
    except Exception as e:
        log(f"[BASS] Device enumeration failed: {e}", 30)
    return results
...
class BassEngine(QObject):
    ...
    def _init_bass(self):
        try:
            ...
            if not self.bass.BASS_Init(dev, 44100, 0, 0, 0):
                log(f"[BASS] Init failed on device {dev}, error code: {self.bass.BASS_ErrorGetCode()}", 40)
                if dev != -1:
                    if self.bass.BASS_Init(-1, 44100, 0, 0, 0):
                        log("[BASS] Fallback to default device succeeded", 30)
            ...
            if os.path.exists(bassmidi_path):
                ...
                if not os.path.exists(sf2):
                    log("[BASS] SoundFont missing, MIDI disabled", 30)
                    self.bassmidi = None
                else:
                    ...
                    if font_handle:
                        ...
                        log("[BASS] SoundFont loaded", 20)
                    else:
                        log("[BASS] SoundFont init failed", 30)
                        self.bassmidi = None
            else:
                log("[BASS] bassmidi.dll not found", 30)
            log(f"[BASS] Initialized on device index {self._device_index}", 20)
        except OSError as e:
            log(f"[BASS] DLL load error: {e}", 40)
            self.bass = None
        except Exception as e:
            log(f"[BASS] Unexpected error: {e}", 40)
            self.bass = None
    ...
    def play(self, filepath, start_ms=0):
        ...
        if self.handle:
            ...
            log(f"[BASS] Playing: {filepath} start_ms={start_ms}", 20)
            return True
        log("[BASS] Failed to load file", 40)
        return False
...
class LibOpenMptEngine:
    ...
    def play_to_tempfile(filepath):
        try:
            ...
            log(f"[libopenmpt] FFmpeg error: {e}", 40)
            return None
...
class AsapEngine:
    ...
    def play_to_tempfile(filepath):
        try:
            ...
            log(f"[ASAP] asapconv error: {e}", 40)
            return None
    ...
    def play_to_tempfile(filepath):
        try:
            ...
            if not os.path.exists(zx):
                ...
                if not os.path.exists(zx):
                    raise FileNotFoundError("Neither zxtune-cli.exe nor zxtune123.exe found")
            for cmd in ([zx, '-o', tmp, filepath],
                ...
                try:
                    subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL,
                                   ...
                                   creationflags=subprocess.CREATE_NO_WINDOW)
                    log(f"[ZXTune] Success args: {cmd[1:]}", 20)
                    return tmp
                except subprocess.CalledProcessError:
                    continue
            log("[ZXTune] All variants failed", 40)
            return None
        except Exception as e:
            log(f"[ZXTune] Error: {e}", 40)
            return None
    ...
    def convert_with_furnace(self, filepath):
        try:
            ...
            subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL,
                           stderr=subprocess.DEVNULL,
                           creationflags=subprocess.CREATE_NO_WINDOW)
            log(f"[Furnace] Conversion successful: {filepath}", 20)
            return tmp
        except Exception as e:
            log(f"[Furnace] Error: {e}", 40)
            return None
...
def load_playlist_data():
    ...
    if source_paths is None:
        ...
        if source_paths:
            log("[Data] play_list uses legacy key 'list' -> reading as source_paths", 20)

    favorites = data.get("favorites")
    if favorites is None:
        favorites = data.get("love_lists", {})
        if favorites:
            log("[Data] play_list uses legacy key 'love_lists' -> reading as favorites", 20)
...
def load_cache():
    ...
    for fallback in (OLD_CACHE_JSON, OLD_CACHE_ALT):
        ...
        if isinstance(data, dict):
            log(f"[Migrate] Loaded cache from {fallback}, writing to {CACHE_FILE}", 20)
            save_cache(data)
...
def migrate_old_files_if_needed():
    ...
    log(f"[Migrate] PLAYLIST_FILE = {PLAYLIST_FILE}", 20)
    log(f"[Migrate] HISTORY_FILE  = {HISTORY_FILE}", 20)
    log(f"[Migrate] CACHE_FILE    = {CACHE_FILE}", 20)
    ...
    if all_new_exist:
        log("[Migrate] All primary data files exist, no migration needed", 20)
    ...
    for candidate in (OLD_DATA_JSON, OLD_DATA_JSON_ALT):
        if os.path.exists(candidate):
            ...
            if isinstance(d, dict):
                ...
                log(f"[Migrate] {candidate} exists but is not valid JSON", 30)
    ...
    if old_data is None:
        ...
        if not legacy_sources and not legacy_favorites:
            ...
            if list_src or love_src:
                ...
                log(f"[Migrate] Legacy txt: list={list_src}, love={love_src}", 20)
    ...
    if migrated_any:
        ...
        log("[Migrate] No new files were written, old kept in place", 20)
...
class MusicRoom(QMainWindow):
    ...
    def __init__(self):
        ...
        log(f"Settings: theme={self.current_theme_index}, loop={self.loop_mode}, "
            ...
            f"bass_device='{self.bass_device}', bg='{self.custom_background_path}'", 20)
        ...
        try:
            ...
            self.vlc_ok = True
            log("VLC initialized", 20)
        except Exception as e:
            log(f"VLC init FAILED: {e}", 40)
        ...
        if self.vlc_device and self.vlc_player is not None:
            if vlc_set_audio_device(self.vlc_player, self.vlc_device):
                log(f"[Audio] VLC using device '{self.vlc_device}'", 20)
            else:
                log(f"[Audio] VLC device '{self.vlc_device}' failed, fallback to default", 30)
                self.vlc_device = ""
        ...
        try:
            ...
            if self.bass_available:
                ...
                log(f"[Audio] BASS using device index {bass_index}", 20)
            else:
                log("[Audio] BASS unavailable", 30)
        except Exception as e:
            log(f"BASS creation FAILED: {e}", 40)
    ...
    def _get_vlc_device_list(self):
        ...
        try:
            ...
            log(f"[VLC] Device list failed: {e}", 30)
            return []
    ...
    def _main_output_device_label(self):
        ...
        if self.vlc_device == self.bass_device:
            ...
            return self.vlc_device or "System Default"
        return "Mixed"
    ...
    def _switch_output_device(self, vlc_id, bass_id):
        log(f"[Audio] Switch request: vlc='{vlc_id}', bass='{bass_id}'", 20)
        ...
        try:
            ...
            if vlc_id:
                ...
                if not ok:
                    log(f"[Audio] VLC failed to set device '{vlc_id}', using default", 30)
                    vlc_id = ""
            log(f"[Audio] VLC player rebuilt, device='{vlc_id}'", 20)
        except Exception as e:
            log(f"[Audio] VLC rebuild failed: {e}", 40)
            self.vlc_ok = False
            vlc_id = ""
        ...
        try:
            ...
            if not self.bass_available:
                log("[Audio] BASS rebuild unavailable", 30)
            else:
                log(f"[Audio] BASS rebuilt, device index={bass_index}", 20)
        except Exception as e:
            log(f"[Audio] BASS rebuild failed: {e}", 40)
            self.bass_available = False
    ...
    def pick_main_device(self):
        ...
        dlg = DevicePickerDialog(self, "Output Device (VLC + BASS)", devices,
    ...
    def pick_vlc_device(self):
        devices = self._get_vlc_device_list()
        dlg = DevicePickerDialog(self, "VLC Device", devices,
                                 self.accent_color, self.dos_font, current_id=self.vlc_device)
        if dlg.exec() == QDialog.DialogCode.Accepted:
            new_id = dlg.selected_id or ""
    ...
    def pick_bass_device(self):
        ...
        dlg = DevicePickerDialog(self, "BASS Device", items,
    ...
    def _resolve_background_image_source(self):
        """Return an existing background image path, or None."""
        if self.custom_background_path and os.path.isfile(self.custom_background_path):
    ...
    def apply_background_image(self):
        ...
        if self.background_mode == "image":
            src = self._resolve_background_image_source()
            if src:
                self.bg_pixmap = QPixmap(src)
    ...
    # ================== CUSTOM BACKGROUND ==================
    def _custom_bg_display_label(self):
        if self.custom_background_path and os.path.isfile(self.custom_background_path):
            return "custom"
        return "built-in"

    def _delete_custom_background_file(self):
        if os.path.isfile(CUSTOM_BG_PATH):
            try:
                os.remove(CUSTOM_BG_PATH)
                log(f"[Background] Deleted {CUSTOM_BG_PATH}", 20)
            except Exception as e:
                log(f"[Background] Failed to delete {CUSTOM_BG_PATH}: {e}", 30)

    def pick_background_image(self):
        src, _filter = QFileDialog.getOpenFileName(
            self, "Select PNG background image", "",
            "PNG image (*.png)"
        )
        if not src:
            return
        ...
        if not src.lower().endswith(".png"):
            QMessageBox.warning(self, "Invalid file", "Only PNG files are accepted.")
            return
        ...
        try:
            ...
            return
        if size_bytes > CUSTOM_BG_MAX_BYTES:
            QMessageBox.warning(
                self, "File too large",
                f"PNG must be at most {CUSTOM_BG_MAX_BYTES // (1024*1024)} MB "
                f"(this file is {size_bytes / (1024*1024):.1f} MB)."
            )
            return
        ...
        if not reader.canRead():
            QMessageBox.warning(self, "Invalid file", "Not a readable PNG image.")
            return
        img_size = reader.size()
        w = img_size.width()
        h = img_size.height()
        if w <= 0 or h <= 0:
            QMessageBox.warning(self, "Invalid file", "PNG has invalid dimensions.")
            return
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Cannot write",
                f"Cannot create background folder:\n{BACKGROUND_DIR}\n\n{e}\n\n"
                f"Please move the program to a writable location."
            )
            return

        need_resize = (w > CUSTOM_BG_MAX_DIM) or (h > CUSTOM_BG_MAX_DIM)

        try:
            if need_resize:
                img = QImage(src)
                if img.isNull():
                    QMessageBox.warning(self, "Invalid file", "Failed to load PNG.")
                    return
                # scale to fit within CUSTOM_BG_MAX_DIM on both axes
                max_dim = CUSTOM_BG_MAX_DIM
                if w >= h:
                    new_w = max_dim
                ...
                img = img.scaled(new_w, new_h,
                                 ...
                                 Qt.TransformationMode.SmoothTransformation)
                ok = img.save(CUSTOM_BG_PATH, "PNG")
                if not ok:
                    QMessageBox.warning(self, "Save failed", "Could not save background PNG.")
                    return
            else:
                shutil.copyfile(src, CUSTOM_BG_PATH)
        except Exception as e:
            QMessageBox.warning(self, "Save failed", f"Could not save background PNG:\n{e}")
            return

        # verify saved size
        try:
            new_size = os.path.getsize(CUSTOM_BG_PATH)
        except Exception:
            new_size = 0
        if new_size > CUSTOM_BG_MAX_BYTES:
            try:
                os.remove(CUSTOM_BG_PATH)
            except Exception:
                pass
            QMessageBox.warning(
                self, "File too large",
                f"Saved PNG exceeds {CUSTOM_BG_MAX_BYTES // (1024*1024)} MB.\n"
                f"Please choose a smaller image."
            )
            return

        self.custom_background_path = CUSTOM_BG_PATH
        self.background_mode = "image"
        self.apply_background_image()
        self._save_settings()
        self.update()
        self.update_settings_list()
        self.track_status.setText("Background image updated")
        log(f"[Background] Custom background set: {CUSTOM_BG_PATH} "
            f"({new_size / 1024:.0f} KB, {w}x{h}"
            f"{' -> resized' if need_resize else ''})", 20)
    ...
    def clear_background_image(self):
        ...
        self._delete_custom_background_file()
        self.apply_background_image()
        self._save_settings()
        self.update()
        self.update_settings_list()
        self.track_status.setText("Custom background cleared")
        log("[Background] Custom background cleared", 20)
    ...
    def update_settings_list(self):
        ...
        if self.show_advanced_audio:
            self.settings_list.addItem(f"  VLC Device                         [{self._vlc_device_label()}]")
            self.settings_list.addItem(f"  BASS Device                        [{self._bass_device_label()}]")
            self.settings_list.addItem("  << Hide Advanced Audio")
        else:
        ...
        bg_label = self._custom_bg_display_label()
        self.settings_list.addItem(f"Custom Background                    [{bg_label}]")
        ...
        engine_label = {ENGINE_MODE_NORMAL: "Normal", ENGINE_MODE_VLC: "VLC", ENGINE_MODE_BASS: "BASS"}.get(self.engine_mode, "Normal")
        self.settings_list.addItem(f"Engine Mode                          [{engine_label}]")
    ...
    def on_setting_selected(self, item):
        ...
        elif "Advanced Audio >>" in text:
            self.show_advanced_audio = True
            self._save_settings()
        elif "<< Hide Advanced Audio" in text:
            self.show_advanced_audio = False
            self._save_settings()
        elif "VLC Device" in text:
            self.pick_vlc_device()
        elif "BASS Device" in text:
        ...
        elif text.startswith("Engine Mode"):
    ...
    def toggle_engine_mode(self):
        if self.engine_mode == ENGINE_MODE_NORMAL:
            ...
            self.engine_mode = ENGINE_MODE_NORMAL
        self._save_settings()
        log(f"[Settings] Engine mode = {self.engine_mode}", 20)
    ...
    def _build_about_text(self):
        ...
        try:
            ...
            lines.append(f"VLC version : {vlc_ver}")
        except Exception:
            lines.append("VLC version : -")
        ...
        lines.append(f"log dir        : {os.path.join(app_dir(), 'data')}")
        try:
        ...
        lines.append(f"current engine : {self.current_engine}")
        ...
        lines.append(f"  vlc actual   : {self._vlc_device_label()}")
        lines.append(f"  bass actual  : {self._bass_device_label()}")
        lines.append("")
        ...
        bg_custom = "yes" if (self.custom_background_path and os.path.isfile(self.custom_background_path)) else "no"
        lines.append(f"custom bg      : {bg_custom}")
        ...
        lines.append(f"log enabled    : {LOG_ENABLED}")
    ...
    def cycle_background_mode(self):
        if self.background_mode == "transparent":
            ...
            if self._resolve_background_image_source():
                self.background_mode = "image"
            else:
                self.background_mode = "transparent"
        else:
    ...
    def set_fullscreen_mode(self):
        ...
        self.setFontSize(12)
        self.apply_font_to_all()
        self.track_title.fixed_visible_chars = 65
        self.track_title.setText(self.track_title.full_text)
        self.text_eq.fix_widths(self.dos_font)
    ...
    def set_mini_mode(self):
        ...
        self.setFontSize(9)
        self.apply_font_to_all()
        self.track_title.fixed_visible_chars = 35
        self.track_title.setText(self.track_title.full_text)
        self.text_eq.fix_widths(self.dos_font)
        self.progress_text.setBarLength(18)
    ...
    def play_file(self, track_or_path):
        ...
        if cache_key in self.convert_cache:
            ...
            if os.path.exists(cached_wav):
                ...
                if self.vlc_player is None:
                    self.track_status.setText("Cannot play (no VLC)")
                    return
                media = self.vlc_instance.media_new(cached_wav)
                self.vlc_player.set_media(media)
                if self.vlc_player.play() == -1:
                    self.track_status.setText("Cannot play")
                    return
        ...
        if engine is None:
            self.track_status.setText("Cannot play - required engine missing")
            self._mark_now_playing(None)
            return
        ...
        log(f"[Play] {path} engine={engine} start_ms={track.get('start_ms', 0)} "
        ...
        if engine == 'bass':
            ...
            if success:
                ...
                log(f"[BASS] fallback to VLC for {path}", 30)
                engine = 'vlc'
        if engine == 'vlc':
            if self.vlc_player is None:
                self.track_status.setText("Cannot play (no VLC)")
    ...
    def _check_vlc_playback(self, wav):
        ...
        if state != vlc.State.Playing:
            log("[Conversion] VLC failed, fallback", 30)
            self._play_with_vlc_fallback(self.current_file_path)
        else:
            self.track_status.setText("Playing")
            prevent_sleep(True)
    ...
    def _play_with_vlc_fallback(self, filepath):
        if self.current_engine == 'chiptune_cli':
            self.track_status.setText("Cannot play - VLC cannot play .dnm/.ftm")
            return
        if self.vlc_player is None or self.vlc_instance is None:
            self.track_status.setText("Cannot play")
            return
        if not filepath.startswith("http") and not os.path.isfile(filepath):
            self.track_status.setText("Cannot play - file missing")
            return
        self.vlc_player.stop()
        media = self.vlc_instance.media_new(filepath)
        self.vlc_player.set_media(media)
        if self.vlc_player.play() == -1:
            self.track_status.setText("Cannot play - unsupported format")
        else:
    ...
    def _check_fallback_playback(self):
        ...
        if self.vlc_player.get_state() != vlc.State.Playing:
            self.track_status.setText("Cannot play - VLC failed")
        else:
            self.track_status.setText("Playing (VLC)")
            prevent_sleep(True)
        ...
        if self.current_engine in ('vlc', 'libopenmpt', 'asap', 'zxtune', 'chiptune_cli', 'cached'):
            if self.vlc_player is not None:
                ...
                if state == vlc.State.Playing:
                    ...
                    if self.track_status.text() not in ("Paused", "Playing (cached)"):
                        self.track_status.setText("Playing")
                elif state == vlc.State.Paused:
                    self.track_status.setText("Paused")
                else:
                    if self.track_status.text() not in ("Playing (cached)",):
                        self.track_status.setText("Stopped")
...
if __name__ == "__main__":
    try:
        ...
        log(f"BACKGROUND_DIR = {BACKGROUND_DIR}", 20)
