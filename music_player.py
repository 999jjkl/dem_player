...
class MusicRoom(QMainWindow):
    ...
    def stop_current_engine(self):
        # Mark end as handled so state transitions caused by our own stop()
        # don't accidentally fire on_media_end
    ...
    def play_file(self, track_or_path):
        ...
        if cache_key in self.convert_cache:
            ...
            if os.path.exists(cached_wav):
                ...
                self.update_now_playing_marker()
                # reset handled flag AFTER we've started a new playback
                self._media_end_handled = False
                self._vlc_last_state = None
                return
            else:
        ...
        if engine in ('libopenmpt', 'asap', 'zxtune', 'chiptune_cli'):
            ...
            # reset handled flag now so when the converted WAV ends, loop works
            self._media_end_handled = False
            self._vlc_last_state = None
        ...
        # reset the handled flag now that a new playback has been set up
        self._media_end_handled = False
        self._vlc_last_state = None
    ...
    def on_conversion_finished(self, wav, engine):
        if wav:
            ...
            # allow end-of-track detection for converted playback
            self._media_end_handled = False
            self._vlc_last_state = None
        else:
            self._play_with_vlc_fallback(self.current_file_path)
    ...
    def on_vlc_end(self, event):
        # Called from VLC's event thread. Use QMetaObject.invokeMethod so the
        # queued call is delivered reliably to the Qt main thread under PySide6.
        try:
            QMetaObject.invokeMethod(self, "on_media_end", Qt.ConnectionType.QueuedConnection)
        except Exception:
            QTimer.singleShot(0, self.on_media_end)

    @Slot()
    def on_media_end(self):
        # Guard against double-trigger (event + polling)
    ...
    def update_ui(self):
        ...
        if self.current_engine in ('vlc', 'libopenmpt', 'asap', 'zxtune', 'chiptune_cli', 'cached'):
            if self.vlc_player is not None:
                ...
                # ---- End-of-track detection (fallback if VLC event lost) ----
                if (self._vlc_last_state == vlc.State.Playing
                        and state == vlc.State.Stopped
                        and not self._media_end_handled):
                    log("[VLC] Playing -> Stopped detected via poll, firing on_media_end", 20)
                    QTimer.singleShot(0, self.on_media_end)
                self._vlc_last_state = state
